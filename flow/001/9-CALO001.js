...
router.get('/CHECK-CALO001', async (req, res) => {

  return res.json(CALO001db['PO']);
});


router.post('/CALO001db', async (req, res) => {
  //-------------------------------------
  // console.log('--CALO001db--');
...
router.post('/GETINtoCALO001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoCALO001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
// router.post('/CALO001-geteachITEM', async (req, res) => {
//   //-------------------------------------
//   console.log('--CALO001-geteachITEM--');
//   console.log(req.body);
//   let inputB = req.body;
...
//   for (i = 0; i < CALO001db['ItemPickcode'].length; i++) {
//     if (CALO001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
//       ITEMSS = CALO001db['ItemPickcode'][i]['key'];
//     }
//   }
...
//     //-------------------------------------
//     CALO001db['inspectionItem'] = ITEMSS;
//     CALO001db['inspectionItemNAME'] = inputB['ITEMs'];
//     let input = { 'PO': CALO001db["PO"], 'CP': CALO001db["CP"], 'ITEMs': CALO001db['inspectionItem'] };
//     //-------------------------------------
//     if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
//       let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
//       let UNITdata = await mongodb.find(master_FN, UNIT, {});
//       let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": CALO001db['inspectionItem'] });
...
//             //
//             CALO001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
//             CALO001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
//             //------------------------------------

//             let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
//             CALO001db['GAPnameList'] = [];
//             for (k = 0; k < graph.length; k++) {
//               CALO001db['GAPnameList'].push(graph[k]['NO']);
//             }
//           }

//           for (j = 0; j < UNITdata.length; j++) {
//             if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
//               CALO001db["UNIT"] = UNITdata[j]['UNIT'];
//             }
//           }

//           console.log(findcp[0]['FINAL'][i]['POINT']);

//           CALO001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
//           CALO001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
//           CALO001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

//           CALO001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
//           output = 'OK';
//           let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
//           if (findpo.length > 0) {
//             request.post(
//               'http://127.0.0.1:17270/CALO001-feedback',
//               { json: { "PO": CALO001db['PO'], "ITEMs": CALO001db['inspectionItem'] } },
    ...
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          CALO001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          CALO001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];
          CALO001db["shape"] = findcp[0]['FINAL'][i]['shape']

          CALO001db["SPEC"] = '';
          if (findcp[0]['FINAL'][i]['SPECIFICATIONve'] !== undefined) {
            if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'BTW') {
              CALO001db["SPEC"] = `${findcp[0]['FINAL'][i]['SPECIFICATIONve']['BTW_LOW']}-${findcp[0]['FINAL'][i]['SPECIFICATIONve']['BTW_HI']}`;
            } else if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'HIM(>)') {
              CALO001db["SPEC"] = `>${findcp[0]['FINAL'][i]['SPECIFICATIONve']['HIM_L']}`;
            } else if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'LOL(<)') {
              CALO001db["SPEC"] = `<${findcp[0]['FINAL'][i]['SPECIFICATIONve']['LOL_H']}`;
            } else if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'Actual') {
              CALO001db["SPEC"] = 'Actual';
            }
          }

          CALO001db["K1b"] = findcp[0]['FINAL'][i]['K1b'];
          CALO001db["K1v"] = findcp[0]['FINAL'][i]['K1v'];
          // "FREQUENCY":"",
          CALO001db["FREQUENCY"] = findcp[0]['FINAL'][i]['FREQUENCY'];
          console.log(CALO001db["FREQUENCY"])

          CALO001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];

          let masterITEMsC = await mongodb.find(master_FN, ITEMs, { "masterID": CALO001db['inspectionItem'] });
          ...
          if (masterITEMsC.length > 0) {
            ...
            if (masterITEMsC[0]['CALCULATE'] !== '') {
              ...
              if (masterCALCULATE.length > 0) {
                ...
                CALO001db["FORMULA"] = masterCALCULATE[0]["FORMULA"]
              }
            }
          }
          ...
          let date = Date.now()
          let REFLOT = await mongodb.find(PATTERN, "referdata", { "MATCP": CALO001db['MATCP'], "ITEMS": ITEMSS, "EXP": { $gt: date } });

          console.log(REFLOT)

          if (REFLOT.length > 0) {
            CALO001db["REFLOT"] = REFLOT[0]['TPKLOT'];
          }
          ...
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:17270/FINAL/CALO001-feedback',
              { json: { "PO": CALO001db['PO'], "ITEMs": CALO001db['inspectionItem'] } },
              function (error, response, body2) {
                if (!error && response.statusCode == 200) {
                  // console.log(body2);
...
router.post('/CALO001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--CALO001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    CALO001db['GAPnameListdata'] = graph[0];//confirmdata
    CALO001db['GAP'] = CALO001db['GAPnameListdata'][`GT${CALO001db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/CALO001-preview', async (req, res) => {
  //-------------------------------------
  // console.log('--CALO001-preview--');
  // console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/CALO001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--CALO001-confirmdata--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    // let datapush = CALO001db['preview'][0]

    // if (CALO001db['RESULTFORMAT'] === 'Graph') {
    //   let pushdata = CALO001db['preview'][0]

    //   pushdata['V5'] = CALO001db['GAP'];
    //   pushdata['V1'] = `${CALO001db['confirmdata'].length + 1}:${pushdata['V1']}`;

    //   CALO001db['confirmdata'].push(pushdata);
    //   CALO001db['preview'] = [];
    //   output = 'OK';
    //   CALO001db['GAP'] = CALO001db['GAPnameListdata'][`GT${CALO001db['confirmdata'].length + 1}`]

    // } else if (CALO001db['RESULTFORMAT'] === 'Number') {

    //   let pushdata = CALO001db['preview'][0]

    //   pushdata['V5'] = CALO001db['confirmdata'].length + 1
    //   pushdata['V1'] = `${CALO001db['confirmdata'].length + 1}:${pushdata['V1']}`
    ...
    //   output = 'OK';
    // }
    if (CALO001db['RESULTFORMAT'] === 'CAL1') {
      ...
      if (input['PO'] != undefined && input['CP'] != undefined && input['VAL1'] != undefined && input['VAL2'] != undefined && input['VAL3'] != undefined && input['VAL4'] != undefined && input['FORMULA'] != undefined) {
        ...
        //   }
        // });
        // CALO001db['confirmdata'][0]['VAL1'] = input['VAL1'];
        // CALO001db['confirmdata'][0]['VAL2'] = input['VAL2'];
        // CALO001db['confirmdata'][0]['VAL3'] = input['VAL3'];
        // CALO001db['confirmdata'][0]['VAL4'] = input['VAL4'];
        // CALO001db['confirmdata'][0]['Area'] = input['Area'];
        // CALO001db['confirmdata'][0]['FORMULA'] = input['FORMULA'];
        // CALO001db['confirmdata'][0]['Result1'] = input['Result1'];
        // CALO001db['confirmdata'][0]['Result2'] = input['Result2'];
        // CALO001db['confirmdata'][0]['Result'] = input['Result'];
        CALO001db['confirmdata'].push({
          'VAL1': input['VAL1'],
          'VAL2': input['VAL2'],
          'VAL3': input['VAL3'],
          'VAL4': input['VAL4'],
...
router.post('/CALO001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--CALO001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      CALO001db["PCSleft"] = `${parseInt(CALO001db["PCS"]) - oblist.length}`;
      if (CALO001db['RESULTFORMAT'] === 'Number' || CALO001db['RESULTFORMAT'] === 'Text' || CALO001db['RESULTFORMAT'] === 'Graph') {
        ...
        CALO001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        CALO001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(CALO001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (CALO001db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (CALO001db['INTERSEC'] !== '') {
                core = parseFloat(CALO001db['INTERSEC'])
              } else {
              ...
              try {
                ...
                CALO001db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (CALO001db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:17270/JUDEMENT", { "PO": CALO001db["PO"], "CP": CALO001db["CP"] })
      ...
      CALO001db["ITEMleftUNIT"] = '';
      CALO001db["ITEMleftVALUE"] = '';
    }
...
router.post('/CALO001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--CALO001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/CALO001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--CALO001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/CALO001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--CALO001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = CALO001db['confirmdata'].length
    if (all > 0) {
      CALO001db['confirmdata'].pop();
    }
...
router.post('/CALO001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--CALO001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (CALO001db['RESULTFORMAT'] === 'Number' || CALO001db['RESULTFORMAT'] === 'Text') {

    CALO001db["value"] = [];
    console.log(CALO001db["value"]);
    for (i = 0; i < CALO001db['confirmdata'].length; i++) {
      CALO001db["value"].push({
        "PO1": CALO001db["inspectionItemNAME"],
        "PO2": CALO001db['confirmdata'][i]['V1'],
        "PO3": CALO001db['confirmdata'][i]['V2'],
        "PO4": CALO001db['confirmdata'][i]['V3'],
        "PO5": CALO001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (CALO001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < CALO001db["value"].length; i++) {
        mean01.push(parseFloat(CALO001db["value"][i]["PO3"]));
        mean02.push(parseFloat(CALO001db["value"][i]["PO5"]));
      }
      ...
      CALO001db["value"].push({
        "PO1": 'Mean',
        "PO2": CALO001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": CALO001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (CALO001db['RESULTFORMAT'] === 'OCR' || CALO001db['RESULTFORMAT'] === 'Picture') {

  } else if (CALO001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < CALO001db['confirmdata'].length; i++) {
      CALO001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": CALO001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (CALO001db['RESULTFORMAT'] === 'Number' ||
    CALO001db['RESULTFORMAT'] === 'Text' ||
    CALO001db['RESULTFORMAT'] === 'OCR' ||
    CALO001db['RESULTFORMAT'] === 'CAL1' ||
    CALO001db['RESULTFORMAT'] === 'Picture' || CALO001db['RESULTFORMAT'] === 'Graph') {
    request.post(
  ...
  //-------------------------------------
  return res.json(CALO001db);
});


router.post('/CALO001-FINISH-CAL1', async (req, res) => {
  //-------------------------------------
  console.log('--CALO001-FINISH-CAL1--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';
  if ((CALO001db['RESULTFORMAT'] === 'CAL1')) {
    ...
    // if (feedback.length > 0) {
    if (input['VAL1'] !== '' && input['VAL2'] !== '' && input['Result'] !== '') {
      CALO001db["value"].push({
        "VAL1": input['VAL1'],
        "VAL2": input['VAL2'],
        "VAL3": input['VAL3'],
        ...
        request.post(
          ...
          function (error, response, body) {
            if (!error && response.statusCode == 200) {
              ...
              request.post(
                'http://127.0.0.1:17270/CALO001-feedback',
                { json: { "PO": CALO001db['PO'], "ITEMs": CALO001db['inspectionItem'], "ITEMsN": CALO001db['inspectionItemNAME'], "Result": input['Result'] } },
                function (error, response, body2) {
                  if (!error && response.statusCode == 200) {
                    // console.log(body2);
                    // if (body2 === 'OK') {
