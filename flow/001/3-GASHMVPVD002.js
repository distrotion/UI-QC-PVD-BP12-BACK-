...
router.get('/CHECK-GASHMVPVD002', async (req, res) => {

  return res.json(GASHMVPVD002db['PO']);
});


router.post('/GASHMVPVD002db', async (req, res) => {
  //-------------------------------------
  // console.log('--GASHMVPVD002db--');
  // console.log(req.body);
  //-------------------------------------
  let finddb = [{}];
  try {

    finddb = GASHMVPVD002db;
    finddbbuffer = finddb;
  }
  catch (err) {
    finddb = finddbbuffer;
...
router.post('/GETINtoGASHMVPVD002', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoGASHMVPVD002--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  check = GASHMVPVD002db;
...
router.post('/GASHMVPVD002-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD002-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < GASHMVPVD002db['ItemPickcode'].length; i++) {
    if (GASHMVPVD002db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = GASHMVPVD002db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    GASHMVPVD002db['inspectionItem'] = ITEMSS;
    GASHMVPVD002db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': GASHMVPVD002db["PO"], 'CP': GASHMVPVD002db["CP"], 'ITEMs': GASHMVPVD002db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": GASHMVPVD002db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            GASHMVPVD002db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            GASHMVPVD002db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            GASHMVPVD002db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              GASHMVPVD002db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              GASHMVPVD002db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          GASHMVPVD002db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          GASHMVPVD002db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          GASHMVPVD002db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          GASHMVPVD002db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:17270/GASHMVPVD002-feedback',
              { json: { "PO": GASHMVPVD002db['PO'], "ITEMs": GASHMVPVD002db['inspectionItem'] } },
    ...
    GASHMVPVD002db["POINTs"] = '',
      GASHMVPVD002db["PCS"] = '',
      GASHMVPVD002db["PCSleft"] = '',
      GASHMVPVD002db["UNIT"] = "",
      GASHMVPVD002db["INTERSEC"] = "",
      output = 'NOK';
  }
...
router.post('/GASHMVPVD002-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD002-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    GASHMVPVD002db['GAPnameListdata'] = graph[0];//confirmdata
    GASHMVPVD002db['GAP'] = GASHMVPVD002db['GAPnameListdata'][`GT${GASHMVPVD002db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/GASHMVPVD002-preview', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD002-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASHMVPVD002-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD002-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = GASHMVPVD002db['preview'][0]

    if (GASHMVPVD002db['RESULTFORMAT'] === 'Graph') {
      let pushdata = GASHMVPVD002db['preview'][0]

      pushdata['V5'] = GASHMVPVD002db['GAP'];
      pushdata['V1'] = `${GASHMVPVD002db['confirmdata'].length + 1}:${pushdata['V1']}`;

      GASHMVPVD002db['confirmdata'].push(pushdata);
      GASHMVPVD002db['preview'] = [];
      output = 'OK';
      GASHMVPVD002db['GAP'] = GASHMVPVD002db['GAPnameListdata'][`GT${GASHMVPVD002db['confirmdata'].length + 1}`]

    } else if (GASHMVPVD002db['RESULTFORMAT'] === 'Number') {

      let pushdata = GASHMVPVD002db['preview'][0]

      pushdata['V5'] = GASHMVPVD002db['confirmdata'].length + 1
      pushdata['V1'] = `${GASHMVPVD002db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/GASHMVPVD002-confirmdata-set', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD002-confirmdata-set--');
  console.log(req.body);
  let input = req.body;
  GASHMVPVD002db['confirmdata'] = input;
  //-------------------------------------
  res.json('ok');
});

router.post('/GASHMVPVD002-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD002-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      GASHMVPVD002db["PCSleft"] = `${parseInt(GASHMVPVD002db["PCS"]) - oblist.length}`;
      if (GASHMVPVD002db['RESULTFORMAT'] === 'Number' || GASHMVPVD002db['RESULTFORMAT'] === 'Text' || GASHMVPVD002db['RESULTFORMAT'] === 'Graph') {
        ...
        GASHMVPVD002db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        GASHMVPVD002db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(GASHMVPVD002db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (GASHMVPVD002db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (GASHMVPVD002db['INTERSEC'] !== '') {
                core = parseFloat(GASHMVPVD002db['INTERSEC'])
              } else {
              ...
              try {
                ...
                GASHMVPVD002db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (GASHMVPVD002db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:17270/JUDEMENT", { "PO": GASHMVPVD002db["PO"], "CP": GASHMVPVD002db["CP"] })
      ...
      GASHMVPVD002db["ITEMleftUNIT"] = '';
      GASHMVPVD002db["ITEMleftVALUE"] = '';
    }
...
router.post('/GASHMVPVD002-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD002fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASHMVPVD002-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD002fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/GASHMVPVD002-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD002fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = GASHMVPVD002db['confirmdata'].length
    if (all > 0) {
      GASHMVPVD002db['confirmdata'].pop();
    }
...
router.post('/GASHMVPVD002-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD002-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (GASHMVPVD002db['RESULTFORMAT'] === 'Number' || GASHMVPVD002db['RESULTFORMAT'] === 'Text') {

    GASHMVPVD002db["value"] = [];
    for (i = 0; i < GASHMVPVD002db['confirmdata'].length; i++) {
      GASHMVPVD002db["value"].push({
        "PO1": GASHMVPVD002db["inspectionItemNAME"],
        "PO2": GASHMVPVD002db['confirmdata'][i]['V1'],
        "PO3": GASHMVPVD002db['confirmdata'][i]['V2'],
        "PO4": GASHMVPVD002db['confirmdata'][i]['V3'],
        "PO5": GASHMVPVD002db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (GASHMVPVD002db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < GASHMVPVD002db["value"].length; i++) {
        mean01.push(parseFloat(GASHMVPVD002db["value"][i]["PO3"]));
        mean02.push(parseFloat(GASHMVPVD002db["value"][i]["PO5"]));
      }
      ...
      GASHMVPVD002db["value"].push({
        "PO1": 'Mean',
        "PO2": GASHMVPVD002db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": GASHMVPVD002db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (GASHMVPVD002db['RESULTFORMAT'] === 'OCR' || GASHMVPVD002db['RESULTFORMAT'] === 'Picture') {

  } else if (GASHMVPVD002db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < GASHMVPVD002db['confirmdata'].length; i++) {
      GASHMVPVD002db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": GASHMVPVD002db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (GASHMVPVD002db['RESULTFORMAT'] === 'Number' ||
    GASHMVPVD002db['RESULTFORMAT'] === 'Text' ||
    GASHMVPVD002db['RESULTFORMAT'] === 'OCR' ||
    GASHMVPVD002db['RESULTFORMAT'] === 'Picture' || GASHMVPVD002db['RESULTFORMAT'] === 'Graph') {
    request.post(
  ...
  //-------------------------------------
  return res.json(GASHMVPVD002db);
});
