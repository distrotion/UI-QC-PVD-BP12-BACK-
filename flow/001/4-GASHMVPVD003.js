...
router.get('/CHECK-GASHMVPVD003', async (req, res) => {

  return res.json(GASHMVPVD003db['PO']);
});


router.post('/GASHMVPVD003db', async (req, res) => {
  //-------------------------------------
  // console.log('--GASHMVPVD003db--');
  // console.log(req.body);
  //-------------------------------------
  let finddb = [{}];
  try {

    finddb = GASHMVPVD003db;
    finddbbuffer = finddb;
  }
  catch (err) {
    finddb = finddbbuffer;
...
router.post('/GETINtoGASHMVPVD003', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoGASHMVPVD003--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  check = GASHMVPVD003db;
...
router.post('/GASHMVPVD003-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD003-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < GASHMVPVD003db['ItemPickcode'].length; i++) {
    if (GASHMVPVD003db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = GASHMVPVD003db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    GASHMVPVD003db['inspectionItem'] = ITEMSS;
    GASHMVPVD003db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': GASHMVPVD003db["PO"], 'CP': GASHMVPVD003db["CP"], 'ITEMs': GASHMVPVD003db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": GASHMVPVD003db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            GASHMVPVD003db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            GASHMVPVD003db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            GASHMVPVD003db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              GASHMVPVD003db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              GASHMVPVD003db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          GASHMVPVD003db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          GASHMVPVD003db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          GASHMVPVD003db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          GASHMVPVD003db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:17270/GASHMVPVD003-feedback',
              { json: { "PO": GASHMVPVD003db['PO'], "ITEMs": GASHMVPVD003db['inspectionItem'] } },
    ...
    GASHMVPVD003db["POINTs"] = '',
      GASHMVPVD003db["PCS"] = '',
      GASHMVPVD003db["PCSleft"] = '',
      GASHMVPVD003db["UNIT"] = "",
      GASHMVPVD003db["INTERSEC"] = "",
      output = 'NOK';
  }
...
router.post('/GASHMVPVD003-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD003-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    GASHMVPVD003db['GAPnameListdata'] = graph[0];//confirmdata
    GASHMVPVD003db['GAP'] = GASHMVPVD003db['GAPnameListdata'][`GT${GASHMVPVD003db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/GASHMVPVD003-preview', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD003-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASHMVPVD003-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD003-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = GASHMVPVD003db['preview'][0]

    if (GASHMVPVD003db['RESULTFORMAT'] === 'Graph') {
      let pushdata = GASHMVPVD003db['preview'][0]

      pushdata['V5'] = GASHMVPVD003db['GAP'];
      pushdata['V1'] = `${GASHMVPVD003db['confirmdata'].length + 1}:${pushdata['V1']}`;

      GASHMVPVD003db['confirmdata'].push(pushdata);
      GASHMVPVD003db['preview'] = [];
      output = 'OK';
      GASHMVPVD003db['GAP'] = GASHMVPVD003db['GAPnameListdata'][`GT${GASHMVPVD003db['confirmdata'].length + 1}`]

    } else if (GASHMVPVD003db['RESULTFORMAT'] === 'Number') {

      let pushdata = GASHMVPVD003db['preview'][0]

      pushdata['V5'] = GASHMVPVD003db['confirmdata'].length + 1
      pushdata['V1'] = `${GASHMVPVD003db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/GASHMVPVD003-confirmdata-set', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD003-confirmdata-set--');
  console.log(req.body);
  let input = req.body;
  GASHMVPVD003db['confirmdata'] = input;
  //-------------------------------------
  res.json('ok');
});

router.post('/GASHMVPVD003-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD003-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      GASHMVPVD003db["PCSleft"] = `${parseInt(GASHMVPVD003db["PCS"]) - oblist.length}`;
      if (GASHMVPVD003db['RESULTFORMAT'] === 'Number' || GASHMVPVD003db['RESULTFORMAT'] === 'Text' || GASHMVPVD003db['RESULTFORMAT'] === 'Graph') {
        ...
        GASHMVPVD003db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        GASHMVPVD003db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(GASHMVPVD003db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (GASHMVPVD003db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (GASHMVPVD003db['INTERSEC'] !== '') {
                core = parseFloat(GASHMVPVD003db['INTERSEC'])
              } else {
              ...
              try {
                ...
                GASHMVPVD003db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (GASHMVPVD003db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
              ...
              let dataCheck = await axios.post("http://localhost:17270/GRAPH-recal", {
                "PO": GASHMVPVD003db["PO"],
                "ITEMs": "ITEMs-5f19aaa2fe12be0020dbd3c2",
                "MODE": "CDE",
                "NAME_INS": "GAS-HMVPVD-003",
                "INTERSEC": ""
              })
            }
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:17270/JUDEMENT", { "PO": GASHMVPVD003db["PO"], "CP": GASHMVPVD003db["CP"] })
      ...
      GASHMVPVD003db["ITEMleftUNIT"] = '';
      GASHMVPVD003db["ITEMleftVALUE"] = '';
    }
...
router.post('/GASHMVPVD003-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD003fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASHMVPVD003-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD003fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/GASHMVPVD003-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD003fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = GASHMVPVD003db['confirmdata'].length
    if (all > 0) {
      GASHMVPVD003db['confirmdata'].pop();
    }
...
router.post('/GASHMVPVD003-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVPVD003-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (GASHMVPVD003db['RESULTFORMAT'] === 'Number' || GASHMVPVD003db['RESULTFORMAT'] === 'Text') {

    GASHMVPVD003db["value"] = [];
    for (i = 0; i < GASHMVPVD003db['confirmdata'].length; i++) {
      GASHMVPVD003db["value"].push({
        "PO1": GASHMVPVD003db["inspectionItemNAME"],
        "PO2": GASHMVPVD003db['confirmdata'][i]['V1'],
        "PO3": GASHMVPVD003db['confirmdata'][i]['V2'],
        "PO4": GASHMVPVD003db['confirmdata'][i]['V3'],
        "PO5": GASHMVPVD003db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (GASHMVPVD003db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < GASHMVPVD003db["value"].length; i++) {
        mean01.push(parseFloat(GASHMVPVD003db["value"][i]["PO3"]));
        mean02.push(parseFloat(GASHMVPVD003db["value"][i]["PO5"]));
      }
      ...
      GASHMVPVD003db["value"].push({
        "PO1": 'Mean',
        "PO2": GASHMVPVD003db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": GASHMVPVD003db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (GASHMVPVD003db['RESULTFORMAT'] === 'OCR' || GASHMVPVD003db['RESULTFORMAT'] === 'Picture') {

  } else if (GASHMVPVD003db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < GASHMVPVD003db['confirmdata'].length; i++) {
      GASHMVPVD003db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": GASHMVPVD003db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (GASHMVPVD003db['RESULTFORMAT'] === 'Number' ||
    GASHMVPVD003db['RESULTFORMAT'] === 'Text' ||
    GASHMVPVD003db['RESULTFORMAT'] === 'OCR' ||
    GASHMVPVD003db['RESULTFORMAT'] === 'Picture' || GASHMVPVD003db['RESULTFORMAT'] === 'Graph') {
    request.post(
  ...
  //-------------------------------------
  return res.json(GASHMVPVD003db);
});
